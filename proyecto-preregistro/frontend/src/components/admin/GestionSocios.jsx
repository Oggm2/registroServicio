...
export default function GestionSocios() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal === 'create') {
        await sociosFormadoresAPI.create({ nombre: nombre.trim() })
        toast.success('Socio formador creado')
      } else {
        await sociosFormadoresAPI.update(editId, { nombre: nombre.trim() })
        toast.success('Socio formador actualizado')
      }
      setModal(null)
      loadSocios()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Socio formador eliminado')
    } catch (err) {
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button className="btn btn-outline btn-sm btn-icon"
                        onClick={() => navigate(`/socios-formadores/${s.id}`)} title="Ver detalle">
                        <HiOutlineEye />
                      </button>
                      <button className="btn btn-outline btn-sm btn-icon"
                      ...
                      <button className="btn btn-danger btn-sm btn-icon"
                        onClick={() => handleDelete(s.id)} title="Eliminar">
                        <HiOutlineTrash />
                      </button>
                    </div>
                  </td>
              ...
              {socios.length === 0 && (
                <tr>
                  <td colSpan={2} className="text-center text-muted" style={{ padding: 'var(--space-2xl)' }}>
                    No hay socios formadores registrados
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="Nombre del socio formador"
